...
async def create_company(db: AsyncSession, user: User, org_id: uuid.UUID, data: CompanyCreate) -> Company:
    await _verify_org_access(db, user, org_id)
    company = Company(organization_id=org_id, **data.model_dump())
    db.add(company)
    await db.flush()
